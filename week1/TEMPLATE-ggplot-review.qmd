...
ggplot(penguins, aes(bill_length_mm, bill_depth_mm, color = species)) +
  geom_point() +
  scale_color_manual(values = c("cyan", "pink", "orange"))
...
ggplot() +
  geom_point(penguins, 
             mapping = aes(bill_length_mm, bill_depth_mm, color = species), alpha = 0.5) +
  geom_point(penguins_summary, mapping = aes(mean_bill_length_mm, mean_bill_depth_mm, fill = species), size = 5, shape = 23)
...
ggplot(penguins, aes(flipper_length_mm, fill = species)) +
  ...
  scale_fill_manual(values = c("cornflowerblue", "darkblue", "skyblue")) +
...
(p1 + (p2 / p3)) +
  plot_annotation(title = "Pre-Penguin Fight Club Stat Graphic")
```